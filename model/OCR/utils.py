...
def get_image_list(horizontal_list, free_list, img, model_height = 64):
    ...
    for box in free_list:
        ...
        image_list.append( (box,crop_img) ) # box = [[x1,y1],[x2,y2],[x3,y3],[x4,y4]]  多个文本框
    ...
    for box in horizontal_list:
        ...
        image_list.append( ( [[x_min,y_min],[x_max,y_min],[x_max,y_max],[x_min,y_max]] ,crop_img) )  # 多个文本框
...
def reformat_input(image):
    ...
    if type(image) == str:
        ...
        # 读取图片
    elif type(image) == bytes:  # 如果图片形式为字节流
        nparr = np.frombuffer(image, np.uint8)  # 用于实现动态数组
        img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)  # 从内存中指定的缓冲区读取图像
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # 转RGB
        img_cv_grey = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # 转GRAY

    elif type(image) == np.ndarray:
        if len(image.shape) == 2: # grayscale # 如果是灰色单色图
            img_cv_grey = image
            img = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)
        elif len(image.shape) == 3 and image.shape[2] == 3: # BGRscale # 如果是3通道图 BGR
            img = image
            img_cv_grey = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        elif len(image.shape) == 3 and image.shape[2] == 4: # RGBAscale # 如果是RGB四通道图
            img = image[:,:,:3]
